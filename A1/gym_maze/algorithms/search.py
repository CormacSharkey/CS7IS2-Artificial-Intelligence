...
# Valid = the neighbour has never been visited before, is not queued up to be searched and there is no wall between the agent's node and the neighbour
...
def depth_first_search(maze: gym.MazeEnv, is_render=True):
    ...
    # While there are nodes in the search stack - there are places to visited
    ...
    # Return the solved status, final path , visited nodes and memory footprint
